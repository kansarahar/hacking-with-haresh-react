...
const createPhysicsTestScene = (canvas) => {

  // -------- physics -------- //
  const frameRate = 60;
  const world = new OIMO.World(2, new OIMO.Vec3(0, -9.8, 0));
  const sphereShape = new OIMO.OSphereGeometry(1);
  ...
  bodyConfig1.position = new OIMO.Vec3(0, 1, 1);
  ...
  const sphereGeometry = new THREE.SphereBufferGeometry(1);
  const sphereColor = new THREE.Color(0x00ffff);
  ...
  const boxColor = new THREE.Color(0xffff00);
